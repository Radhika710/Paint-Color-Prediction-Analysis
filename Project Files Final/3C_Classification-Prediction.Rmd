...
mod_3B1_3A6 <- readr::read_rds("mod_3B1_3A6.rds")
mod_3A7 <- readr::read_rds("mod_7.rds")
mod_3B2_3A7 <- readr::read_rds("mod_3B2_3A7.rds")
```
...
```{r}
pull_lm_coefs_signif <- function(model, top_n = 20) {
  # Extract coefficients and their significance from a linear model
  tidy_coefs <- broom::tidy(model)
  
  # Filter for significant coefficients
  significant_coefs <- tidy_coefs %>%
    filter(p.value < 0.05)
  
  # Arrange by absolute estimate value and select the top N
  top_n_coefs <- significant_coefs %>%
    arrange(desc(abs(estimate))) %>%
    slice_head(n = top_n)
  
  # Return the selected coefficient names
  top_n_coefs$term
}
```
...
mod_3B1_3A6 %>% as_tibble() %>%
  select(all_of(
    mod_3A6 %>% pull_lm_coefs_signif(top_n = 20)
  )) %>%
...
```{r mod_3B3_3Ac2}
mod_3B3_3A10 %>% as_tibble() %>%
  select(all_of(
    mod_3A10 %>% pull_lm_coefs_signif(top_n = 20)
  )) %>%
  # mcmc_areas_ridges(prob=0.6, prob_outer = 0.9) +
  mcmc_intervals() +
  geom_vline(xintercept = 0,linetype="dashed")
...
Hue_ref <- dfiiiA$Hue %>% mean()
viz_grid <- expand.grid(R = R_ref,
                        G = seq(min(dfiiiA$G),max(dfiiiA$G),length.out=50),
                        B = B_ref,
...
```{r}
pred_viz_probs <- predict(model_, newdata = viz_grid)
```
...
viz_enet_df <- viz_grid %>% bind_cols(pred_viz_probs)

viz_enet_df$...8 %>% glimpse()

mean_A <- viz_enet_df$fit
std_dev_A <- sqrt(viz_enet_df$se.fit^2)
upper_ci_A = mean_A + 1.96 * std_dev_A
lower_ci_A = mean_A - 1.96 * std_dev_A
upper_pi_A = viz_enet_df$fit + 1.96 * viz_enet_df$se.fit * 2
lower_pi_A = viz_enet_df$fit - 1.96 * viz_enet_df$se.fit * 2

# Plotting the trends for modA
ggplot(viz_enet_df, aes(x = R)) +
...
viz_enet_df %>% ggplot(aes(x=R, fill=Saturation)) +
  geom_ribbon(aes(ymin = trend_lwr, ymax = trend_upr), alpha=0.6) +
  geom_line(aes(y = mean_A), size=0.5) +
